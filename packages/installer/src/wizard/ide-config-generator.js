  ...
  try {
    for (const ideKey of selectedIDEs) {
      ...
      try {
        ...
        // For Claude Code, also copy .claude/rules and .claude/hooks folders
        if (ideKey === 'claude-code') {
          ...
          const hooksFiles = await copyClaudeHooksFolder(projectRoot);
          createdFiles.push(...hooksFiles);
          if (hooksFiles.length > 0) {
            createdFolders.push(path.join(projectRoot, '.claude', 'hooks'));
            spinner.succeed(`Copied ${hooksFiles.length} hook file(s) to .claude/hooks`);
          } else {
            spinner.info('No hook files to copy');
          }
        }
