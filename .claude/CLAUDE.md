...
npx jest --testPathPattern="installer"
...
npm run validate:agents               # Consistência dos arquivos de agente
npm run validate:manifest             # Manifesto de instalação
npm run validate:parity               # Paridade multi-IDE (Claude/Codex/Gemini)
```

### IDE Sync (rodar após editar arquivos de agente em `.aios-core/development/agents/`)
```bash
npm run sync:ide                      # Sincronizar todos os IDEs
npm run sync:ide:claude               # Somente Claude Code
npm run sync:ide:codex                # Somente Codex CLI
npm run sync:ide:gemini               # Somente Gemini CLI
npm run sync:ide:check                # Verificar drift sem sincronizar
```
...
Este repositório É o framework AIOS — ele instala em projetos de usuários via `npx aios-core install`.

### Módulos Principais (`.aios-core/core/`)

| Módulo | Responsabilidade |
|--------|-----------------|
| `config/` | Cache e carregamento de configs de agentes (YAML) |
| `session/` | Detecção de contexto e carregamento de sessão |
| `elicitation/` | Fluxos interativos de perguntas/respostas para agentes |
| `orchestration/` | Orquestração de workflows e tasks |
| `memory/` | Persistência de memória de agentes |
| `health-check/` | Motor de health check programático (não confundir com `doctor/`) |
| `doctor/` | Sistema de diagnóstico primário (`aios doctor` — 15 checks) |
| `synapse/` | Context engine — gestão de domínios e regras |
| `ids/` | Registro de IDs de entidades |
| `quality-gates/` | Runners de quality gate pré-push |

### Pacotes Publicados (`packages/`)

| Pacote | Descrição |
|--------|-----------|
| `installer/` | Wizard interativo (`@clack/prompts`), file merger, updater |
| `aios-pro-cli/` | CLI de features Pro |
| `gemini-aios-extension/` | Integração com Gemini CLI |
| `aios-install/` | Utilitários de instalação |

### Pipeline de IDE Sync

Os arquivos markdown em `.aios-core/development/agents/` são a **fonte da verdade** dos agentes. O `npm run sync:ide` os propaga para Claude Code (`.claude/`), Codex CLI (`AGENTS.md`, `.codex/skills/`), Gemini CLI (`.gemini/`), e Cursor. O lint-staged dispara `sync:ide` automaticamente ao commitar arquivos de agentes.

### Tecnologia Base

- **JavaScript CommonJS** com type-checking TypeScript (`checkJs: false`, `allowJs: true`)
- Node.js >=18.0.0 (v20+ recomendado)
- Imports absolutos configurados via `tsconfig.json` (`baseUrl: "."`)
- Husky + lint-staged enforça ESLint, Prettier e IDE sync no commit
