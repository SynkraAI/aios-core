...
const SubagentPromptBuilder = require('../../../.aiox-core/core/orchestration/subagent-prompt-builder');
...
describe('SubagentPromptBuilder', () => {
  ...
  describe('constructor', () => {
    test('sets project root and paths', () => {
      ...
      expect(builder.aioxCoreRoot).toBe(path.join('/project', '.aiox-core'));
      expect(builder.paths.agents).toContain(path.join('development', 'agents'));
      expect(builder.paths.tasks).toContain(path.join('development', 'tasks'));
      expect(builder.paths.checklists).toContain(path.join('product', 'checklists'));
      expect(builder.paths.templates).toContain(path.join('product', 'templates'));
    });
  });
