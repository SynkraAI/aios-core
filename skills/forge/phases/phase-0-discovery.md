...
3. **Read `README.md`** or `.aiox/INDEX.md` (first 50 lines) → project context
...
Glob `.aiox/forge-runs/*/state.json`. For each file:
...
- HYBRID: `{cwd}/.aiox/memory/project-context.md`
...
**Check:**
1. Does `{PROJECT_DIR}/.aiox/INDEX.md` exist?
...
> 1. **Setup completo**
>    .aiox/ + docs/stories/ + .claude/CLAUDE.md + memory. Pra projetos que você vai manter e evoluir.
> 2. **Setup mínimo**
...
```
1. Create {PROJECT_DIR}/.aiox/INDEX.md with:
   - Project name (from state.json or user input)
   - Status: Active
   - Created: {today}
   - Stack: {from tech_decisions, if available}

2. Create {PROJECT_DIR}/.aiox/memory/ directory
3. Create {PROJECT_DIR}/.aiox/memory/feedback/ directory

4. Create memory files (same templates as /new-project Passo 2.6):
   - `.aiox/memory/project-context.md` — pre-fill with description, stack from tech_decisions
   - `.aiox/memory/agents-used.md` — empty template for tracking
   - `.aiox/memory/squads-config.md` — empty template for tracking
...
8. Update {AIOS_HOME}/docs/projects/ACTIVE.md:
   - Add row: | {project_name} | Active | {path} | [INDEX]({path}/.aiox/INDEX.md) |
```
...
2. Forge creates .aiox/forge-runs/ automatically when the run starts (no extra setup)
```
...
2. Forge creates .aiox/forge-runs/ automatically when the run starts
```
...
2. Create run folder: `.aiox/forge-runs/{run_id}/`
...
1. **Check for concurrent runs (LOCK):**
   - Check if `.aiox/forge-runs/.lock` exists
...
3. **Create lock file:** Write run_id to `.aiox/forge-runs/.lock`
4. Create run folder: `.aiox/forge-runs/{run_id}/`
