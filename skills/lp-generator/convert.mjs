...
// All imports are local — zero external skill dependencies
...
if (flags.help) {
  console.log(`
lp-generator — Convert Markdown to high-conversion Landing Pages
...
Available Brands (8 curated):
  vercel-noir     Dark monochrome — maximum minimalism
  linear-soft     Dark purple muted — SaaS, tech
  raycast-warm    Dark warm orange — tools, productivity
  specta          Dark creative purple — creators, video
  emerald-noir    Dark green luxury — consulting, finance
  rose-gold       Dark romantic — fashion, beauty, events
  arctic-frost    Light corporate — SaaS, docs
...
Examples:
  node convert.mjs landing.md --brand=vercel-noir --style=gsap --effects=premium
  node convert.mjs landing.md --brand=specta --style=nextjs --effects=full-framer
...
const resolvedInput = resolve(expandTilde(inputPath));
let md;
try {
...
if (!validation.valid) {
  console.warn(`\n  Missing recommended sections: ${validation.missing.join(', ')}`);
...
if (flags['validate-only']) {
  console.log('\nValidation Result:');
  console.log(`  Valid: ${validation.valid ? 'Yes' : 'Missing sections'}`);
...
if (style === 'gsap') {
  ...
  const siteOutput = flags.output
    ? resolve(expandTilde(flags.output))
