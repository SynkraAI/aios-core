...
/**
 * Parse offer section: extract offer items, total, price, guarantee, and pricing tiers.
 * If multiple H3 sub-sections exist (excluding "Garantia"), they become pricing tiers (grid layout).
 * Otherwise, falls back to single offer card layout.
 */
function parseOfferTokens(tokens) {
  ...
  for (const token of tokens) {
    // H3 starts a sub-section: Garantia or pricing tier
    if (token.type === 'heading' && token.depth === 3) {
      const lower = token.text.toLowerCase();
      if (lower.includes('garantia') || lower.includes('guarantee')) {
        if (currentTier) { tiers.push(currentTier); currentTier = null; }
        inGuarantee = true;
        continue;
      }
      // New pricing tier
      ...
      currentTier = { name: token.text, items: [], price: '', total: '', badge: '', highlighted: false };
      // Detect highlighted tier: ### **Pro** or ### Pro ⭐
      if (token.text.includes('⭐') || token.raw.includes('**')) {
        currentTier.highlighted = true;
        currentTier.name = token.text.replace(/⭐/g, '').replace(/\*\*/g, '').trim();
      }
      continue;
    }
    ...
    if (token.type === 'paragraph') {
      ...
      for (const line of lines) {
        ...
        if (offerMatch) {
          const item = parseOfferItem(offerMatch[1].trim());
          if (currentTier) { currentTier.items.push(item); } else { items.push(item); }
          continue;
        }
        ...
        if (totalMatch) {
          if (currentTier) { currentTier.total = totalMatch[1].trim(); } else { total = totalMatch[1].trim(); }
          continue;
        }

        // **HOJE:** or **Preço:** price line
        const priceMatch = line.match(/\*\*(?:HOJE|Preço|Preco|Price):\*\*\s*(.*)/i);
        if (priceMatch) {
          if (currentTier) { currentTier.price = priceMatch[1].trim(); } else { price = priceMatch[1].trim(); }
          continue;
        }

        // *Badge text* inside a tier
        const badgeMatch = line.match(/^\*([^*]+)\*\s*$/);
        if (badgeMatch && currentTier) {
          currentTier.badge = badgeMatch[1].trim();
          continue;
        }
  ...
  return { items, total, price, guarantee, paragraphs, tiers };
}
