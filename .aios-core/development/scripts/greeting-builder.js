...
class GreetingBuilder {
  ...
  async _buildContextualGreeting(agent, context) {
    ...
    sections.push(this.buildPresentation(agent, sessionType, permissionBadge, userProfile));
    ...
    sections.push(this.buildFooter(agent, userProfile));
   ...
   * @param {string} userProfile - User profile ('bob' | 'advanced') (optional)
   * @returns {string} Presentation text
   */
  buildPresentation(agent, sessionType, permissionBadge = '', userProfile = DEFAULT_USER_PROFILE) {
    const profile = agent.persona_profile;
    ...
    // Bob mode: use bob-specific greetings for PM agent
    if (userProfile === 'bob' && agent.id === 'pm' && greetingLevels.bob) {
      const bobGreeting =
        greetingLevels.bob.archetypal || greetingLevels.bob.named || greetingLevels.bob.minimal;
      return permissionBadge ? `${bobGreeting} ${permissionBadge}` : bobGreeting;
    }
...
**Para interagir com Bob:**
   ...
   */
  buildFooter(agent, userProfile = DEFAULT_USER_PROFILE) {
    const parts = ['Type `*guide` for comprehensive usage instructions.'];

    // Add agent signature if available
    if (agent?.persona_profile?.communication) {
      const comm = agent.persona_profile.communication;
      // Bob mode: use bob signature for PM agent
      const signature =
        userProfile === 'bob' && agent.id === 'pm' && comm.bob_signature_closing
          ? comm.bob_signature_closing
          : comm.signature_closing;
